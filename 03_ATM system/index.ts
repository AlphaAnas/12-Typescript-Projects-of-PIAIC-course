...
const main = async () => {
  ...
  if (user_id === -1 || !user_exists) {
    return chalk.cyan.bold(
      "The requested user does not exists! Please try again. "
    );
  }
    ...
    case "1. check balance":
      console.log(
        chalk.black.bgWhite.bold(
          `      Your total balance is : ${user.amount} `
        )
      );
      break;
...
const validate_user = async (): Promise<[number, boolean]> => {
  ...
  if (typeof user_data.user_question1 !== "string") {
    console.error(
      chalk.red.bold("Error : Invalid Input, please enter valid name!")
    );
    return [-1, false];
  }
  if (isNaN(Number(user_data.user_question2))) {
    console.error(
      chalk.red.bold("Invalid input, Please enter a valid account number!")
    );
    return [-1, false];
  }
  if (
    user_data.user_question3.length !== 6 ||
    typeof user_data.user_question3 !== "string"
  ) {
    console.error(
      chalk.red.bold("Invalid input, Please enter a valid pin of 6 digits!")
    );
    return [-1, false];
  }
  for (let i = 0; i < users.length; i++) {
    if (
      user_data.user_question1 === users[i].name.toLowerCase() &&
